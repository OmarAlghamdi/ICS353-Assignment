...
function iterative
{
    ...
    & java.exe matrixMultiply 0 $n 0 $inputDir/$fileName $resultDir/"iterative-1-$fileName"
}
function Strassen
{
    param([string]$fileName, [int32]$base, [int32]$n)
    & java.exe matrixMultiply 1 $n $base $inputDir/$fileName $resultDir/"Strassen-$base-$fileName"
}
